...
public class ShannonFano {
    ...
    String encodedDataStringCheck;

    public ShannonFano(ArrayList<Symbol> symbols) {
        ...
        this.symbols.sort((a, b) -> Double.compare(b.getProbability(), a.getProbability()));
        generateCode(this.symbols);
    }

    private void generateCode(List<Symbol> symbols) {
        if(symbols.size() == 1) {
            return;
        }
        ...
        for(int i = 0; i < symbols.size() - 1; i++) {
            sumLeft += symbols.get(i).getProbability();
        ...
        for (int i = 0; i < symbols.size(); i++) {
            if(i <= minDiffIndex) {
                symbols.get(i).setCode(symbols.get(i).getCode() + "0");
            }
            else {
                symbols.get(i).setCode(symbols.get(i).getCode() + "1");
            }
        }

        generateCode(symbols.subList(0, minDiffIndex + 1));
        generateCode(symbols.subList(minDiffIndex + 1, symbols.size()));
    }
